...
test("uses independently isolated assets at the original 1920x1068 coordinates", async () => {
  ...
  assert.match(hero, /liftRotation: -0\.46/);
  assert.match(hero, /liftRotation: 0\.38/);
  assert.match(hero, /bodyLeanLimit: 0\.12/);
  assert.match(hero, /preparationBlend: 0\.28/);
  assert.match(hero, /let actorRotation = 0/);
  assert.match(hero, /actorRotation = readyRotation/);
  assert.match(hero, /liftSpeedLimit: 18/);
  assert.match(hero, /preparationSpeedLimit: 22/);
